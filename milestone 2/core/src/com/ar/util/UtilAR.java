...
public class UtilAR {
	...
	//Properties
	public static boolean alwaysUseImageByteStream = false;
	...
	private static ByteBuffer imData = ByteBuffer.allocateDirect(MAX_IMAGE_BYTES);
	private static OrthographicCamera bgCam = new OrthographicCamera();
	private final static double[] homLine = {0,0,0,1};

	//Temps
	private static byte[] imArray = new byte[MAX_IMAGE_BYTES];
	private static int[] imArrayInt = new int[MAX_IMAGE_BYTES];
	 ...
	 */
	public static Mat getDefaultIntrinsics(float camImgWidth,float camImgHeight) {
		float facX = camImgWidth/640f;
		float facY = camImgHeight/480f;
		intrinsicParams.put(0,0, 670*facX,0,camImgWidth/2, 0,670*facY,camImgHeight/2, 0,0,1);
	...
	/**
	 * Converts the given field of view angle into focal length in terms of pixels.
	 * @param fov the horizontal or vertical field of view angle
	 * @param pixels the width or height of the camera image in pixels
	 * @return The focal length in terms of pixels
	 */
	...
	/**
	 * Converts the given focal length in terms of pixels into field of view angle
	 * @param focalLen the horizontal or vertical focal length in terms of pixels
	 * @param pixels the width or height of the camera image in pixels
	...
	public static Texture createCameraTexture(VideoCapture camera) {
		int width = (int)camera.get(Videoio.CV_CAP_PROP_FRAME_WIDTH);
		int height = (int)camera.get(Videoio.CV_CAP_PROP_FRAME_HEIGHT);
	...
	public static void texDrawBackground(Texture tex) {
		...
		bgCam.setToOrtho(false, 2*ratio,2);
		batch.begin();
		batch.setProjectionMatrix(bgCam.projection);
